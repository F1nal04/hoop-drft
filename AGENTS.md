...
To keep that runner image small, `package.json` splits deps by **runtime vs build**: `dependencies` holds only what runs at production runtime (`next`, `react`, `react-dom`, `ws`, `nanoid`); everything compiled into `.next` (Radix, recharts, Tailwind, etc.) lives in `devDependencies`. The Docker prod-deps stage installs with `--production --omit=optional`, which also drops next's optional native binaries (`@next/swc-*` is build/dev-only; sharp is for `next/image`, which the app doesn't use). **A new runtime import in `server.mjs`/`server/` must go in `dependencies`**, and adopting `next/image` would mean shipping sharp again.
